...
const Export = (() => {
  ...
  function buildAIPackage(record, records, planVersion) {
    ...
    const lines = [
      ...
      `干燥管理: ${scoreLabel("scrotal_moisture", other?.scrotal_moisture)}`,
      "",
      "【体态】",
      ...
      `7日平均干燥管理: ${avg7(r => r.other?.scrotal_moisture)}/5`,
  ...
  function buildMarkdown(record) {
    ...
    return [
      ...
      `- 口干 ${other?.dry_mouth || "—"}/5  干燥管理 ${other?.scrotal_moisture || "—"}/5`,
      "",
      `## 完成率: ${rate}%`,
      "",
      `## 备注`,
